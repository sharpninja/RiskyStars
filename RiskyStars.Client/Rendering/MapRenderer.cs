...
public class MapRenderer
{
    ...
    private void DrawStarSystem(SpriteBatch spriteBatch, StarSystemData system)
    {
        ...
        DrawSystemOrbits(spriteBatch, system);
        DrawCircle(spriteBatch, system.Position, 80f, systemColor, 3f);
        DrawFilledCircle(spriteBatch, system.Position, system.Type == StarSystemType.Featured ? 10f : 8f, systemColor * 0.9f);
        DrawFilledCircle(spriteBatch, system.Position, system.Type == StarSystemType.Featured ? 4f : 3f, Color.White * 0.95f);
    ...
    private void DrawSystemOrbits(SpriteBatch spriteBatch, StarSystemData system)
    {
        if (_pixelTexture == null)
        {
            return;
        ...
        var orbitRadii = system.StellarBodies
            .Select(body => Vector2.Distance(system.Position, body.Position))
            .Where(radius => radius > 5f)
            .OrderBy(radius => radius)
            .ToList();

        float? previousRadius = null;
        foreach (var radius in orbitRadii)
        {
            if (previousRadius.HasValue && Math.Abs(previousRadius.Value - radius) < 6f)
            {
                continue;
            }

            DrawCircle(spriteBatch, system.Position, radius, new Color(95, 120, 95) * 0.38f, 1f);
