...
public class DockableWindow
{
    ...
    protected static int DockThreshold => ThemeManager.ScalePixels(50);
    protected static int TitleBarHeight => ThemeManager.ScalePixels(30);
    protected static int EdgeMargin => ThemeManager.ScalePixels(10);
    ...
    public DockableWindow(string windowId, string title, WindowPreferences preferences, int screenWidth, int screenHeight, int defaultWidth, int defaultHeight)
    {
        ...
        int scaledDefaultWidth = ThemeManager.ScalePixels(defaultWidth);
        int scaledDefaultHeight = ThemeManager.ScalePixels(defaultHeight);
        
        _window = new Window
        {
            Title = title
        };
        ThemeManager.ApplyGameplayWindowTheme(_window);
        ...
        {
            _window.Width = scaledDefaultWidth;
            _window.Height = scaledDefaultHeight;
            _window.Left = (screenWidth - scaledDefaultWidth) / 2;
            _window.Top = (screenHeight - scaledDefaultHeight) / 2;
        }
    ...
    public virtual void ResizeViewport(int screenWidth, int screenHeight)
    {
        ...
        int width = _window.Width ?? 300;
        int height = _window.Height ?? 400;
        int maxTop = Math.Max(TitleBarHeight + EdgeMargin, _screenHeight - height - EdgeMargin);
        int maxLeft = Math.Max(EdgeMargin, _screenWidth - width - EdgeMargin);

        _window.Left = Math.Clamp(_window.Left, EdgeMargin, maxLeft);
        _window.Top = Math.Clamp(_window.Top, TitleBarHeight + EdgeMargin, maxTop);
    }
}
