...
public class CombatScreen
{
    ...
    public CombatPresentation? GetPresentation()
    {
        if (!IsActive || _currentCombatEvent == null)
        {
            return null;
        ...
        var title = _currentCombatEvent.EventType switch
        {
        ...
        var attackers = _currentCombatEvent.ArmyStates
            .Where(a => a.CombatRole == "Attacker")
            .Select(a => $"{a.PlayerId}: {a.UnitCount} units")
            .ToList();

        var defenders = _currentCombatEvent.ArmyStates
            .Where(a => a.CombatRole == "Defender")
            .Select(a => $"{a.PlayerId}: {a.UnitCount} units")
            .ToList();

        var attackerRolls = _animatedAttackerRolls
            .Select((roll, index) => $"A{index + 1}: {(roll.IsRevealed ? roll.DisplayValue.ToString() : "?")}")
            .ToList();

        var defenderRolls = _animatedDefenderRolls
            .Select((roll, index) => $"D{index + 1}: {(roll.IsRevealed ? roll.DisplayValue.ToString() : "?")}")
            .ToList();

        var pairings = _animatedPairings
            .Where(pairing => pairing.IsRevealed)
            .Select((pairing, index) =>
            {
                ...
                string status = p.IsDiscarded ? " [discarded]" : string.Empty;
                return $"{index + 1}. Attacker {attackRoll} vs Defender {defendRoll}{status}";
            })
            .ToList();

        var casualties = _animatedCasualties
            .Where(casualty => casualty.IsRevealed)
            .Select(casualty =>
            {
                var c = casualty.Casualty;
                return $"{c.PlayerId} ({c.CombatRole}): -{c.Casualties} units ({c.RemainingUnits} remaining)";
            })
            .ToList();

        var survivors = _currentCombatEvent.ArmyStates
            .Where(army => army.UnitCount > 0)
            .Select(army => $"{army.PlayerId}: {army.UnitCount} units")
            .ToList();

        return new CombatPresentation
        {
            Title = title,
            Location = _currentCombatEvent.LocationId,
            Round = _currentCombatEvent.RoundResults.Count > 0
                ? $"Round {_currentRoundIndex + 1} of {_currentCombatEvent.RoundResults.Count}"
                : "Awaiting combat rounds",
            Status = GetAnimationStatusText(),
            ReinforcementMessage = _showReinforcementMessage
                ? "New reinforcements have arrived at the battlefield."
                : null,
            Instructions = _animationState == CombatAnimationState.ShowingOutro
                ? "Press ENTER or SPACE to close. ESC skips."
                : "Press ENTER or SPACE to advance. ESC skips.",
            Attackers = attackers,
            Defenders = defenders,
    ...
    private string GetAnimationStatusText()
    {
        return _animationState switch
        {
            CombatAnimationState.ShowingIntro => "Assessing forces",
            CombatAnimationState.RollingDice => "Resolving dice",
            CombatAnimationState.ShowingRolls => "Dice revealed",
            CombatAnimationState.ShowingPairings => "Comparing roll pairings",
            CombatAnimationState.ShowingCasualties => "Applying casualties",
            CombatAnimationState.RoundComplete => "Round complete",
            CombatAnimationState.ShowingOutro => "Combat complete",
            _ => "Combat pending"
        };
    }
...
public sealed class CombatPresentation
{
    ...
    public string? ReinforcementMessage { get; init; }
    public required string Instructions { get; init; }
    public required IReadOnlyList<string> Attackers { get; init; }
    public required IReadOnlyList<string> Defenders { get; init; }
    public required IReadOnlyList<string> AttackerRolls { get; init; }
    public required IReadOnlyList<string> DefenderRolls { get; init; }
    public required IReadOnlyList<string> Pairings { get; init; }
    public required IReadOnlyList<string> Casualties { get; init; }
    public required IReadOnlyList<string> Survivors { get; init; }
}
